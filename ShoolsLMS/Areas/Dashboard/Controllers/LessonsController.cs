...
namespace ShoolsLMS.Areas.Dashboard.Controllers
{
    public class LessonsController : Controller
    {
        ...
        public LessonsController(ApplicationUserManager userManager)
        {
        ...
        {
            LessonListingModel model = new LessonListingModel();
            ...
            model.Lessons = SearchLesson(searchTerm, subjectID, page.Value, recordSize);
        ...
        public int SearchLessonCount(string searchTerm)
        {
            ...
            var subjects = context.Lessons.AsQueryable();

            if (!string.IsNullOrEmpty(searchTerm))
            {
                subjects = subjects.Where(a => a.LessonName.ToLower().Contains(searchTerm.ToLower()));
            }
        ...
        public IEnumerable<Lesson> GetAllLessons()
        {
            ...
            return context.Lessons.Include(l => l.Subject).Include(e => e.User).ToList();
        }
        public IEnumerable<Subject> GetAllSubjects()
        {
            var context = new ApplicationDbContext();

            return context.Subjects.Include(l => l.Lessons).ToList();
        }


        public IEnumerable<Lesson> SearchLesson(string searchTerm,int? assignID, int page, int recordSize)
        {
            var context = new ApplicationDbContext();
            var assignment = context.Lessons.Include(l => l.Subject).Include(e => e.User).AsQueryable();

            if (!string.IsNullOrEmpty(searchTerm))
            {
                assignment = assignment.Where(a => a.LessonName.ToLower().Contains(searchTerm.ToLower()));
            }

            if (assignID.HasValue && assignID.Value > 0)
            {
                assignment = assignment.Where(a => a.LessonId == assignID.Value);
            }


            var skip = (page - 1) * recordSize;

            return assignment.OrderBy(t => t.LessonId == assignID).Skip(skip).Take(recordSize).ToList();
        }
        ...
        //var lessons = db.Papers.Where(e => e.Subject.ApplicationUserID == getuser).
        //  var searchLessons = from l in db.Lessons where l.ApplicationUserID == getuser && l.LessonName.Contains(searchPhrase) select l;

        //  return View("Index", searchPhrase);
        //}
